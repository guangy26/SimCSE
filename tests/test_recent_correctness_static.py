...
class RecentTrainingCorrectnessTests(unittest.TestCase):
    ...
    def test_helper_similarity_mask_is_neutral_except_false_negatives(self):
        source = read_source("train.py")
        ...
        self.assertIn("similarity_mask = torch.ones_like(raw_similarity)", source)
        self.assertIn("raw_similarity > self.similarity_threshold_high", source)
        self.assertIn("similarity_mask.fill_diagonal_(1.0)", source)
        self.assertNotIn("Similarity(0.05)", source)
        self.assertNotIn("similarity_mask = raw_similarity.clone()", source)
