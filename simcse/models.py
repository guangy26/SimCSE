    ...
    if dist.is_initialized() and cls.training:
        ...
        if similarity_mask is not None:
            ...
            global_similarity_mask = torch.ones(
                (local_rows * world_size, local_cols * world_size),
                dtype=similarity_mask.dtype,
                device=similarity_mask.device,
            )
            for process_rank, rank_similarity_mask in enumerate(similarity_mask_list):
                row_start = process_rank * local_rows
                col_start = process_rank * local_cols
