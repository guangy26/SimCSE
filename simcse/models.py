    ...
    if dist.is_initialized() and cls.training:
        ...
        if similarity_mask is not None:
            ...
            similarity_mask_list = [
                torch.ones_like(local_similarity_mask) for _ in range(world_size)
            ]
            dist.all_gather(
                tensor_list=similarity_mask_list,
                tensor=local_similarity_mask,
            )
            similarity_mask_list[rank] = local_similarity_mask
            global_similarity_mask = local_similarity_mask.new_ones(
                batch_size * world_size,
                batch_size * world_size,
            )
            for process_rank, process_similarity_mask in enumerate(similarity_mask_list):
                start = process_rank * batch_size
                end = start + batch_size
                global_similarity_mask[start:end, start:end] = process_similarity_mask
