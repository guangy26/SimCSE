...
def cl_forward(cls,
    ...
    if dist.is_initialized() and cls.training:
        ...
        if similarity_mask is not None:
            local_similarity_mask = similarity_mask
            similarity_mask_list = [torch.ones_like(local_similarity_mask) for _ in range(world_size)]
            dist.all_gather(tensor_list=similarity_mask_list, tensor=local_similarity_mask.contiguous())
            similarity_mask_list[rank] = local_similarity_mask
            similarity_mask = torch.ones(
                z1.size(0),
                z2.size(0),
                dtype=local_similarity_mask.dtype,
                device=local_similarity_mask.device,
            )
            for rank_index, rank_similarity_mask in enumerate(similarity_mask_list):
                row_start = rank_index * batch_size
                row_end = row_start + rank_similarity_mask.size(0)
                col_start = rank_index * batch_size
                col_end = col_start + rank_similarity_mask.size(1)
                similarity_mask[row_start:row_end, col_start:col_end] = rank_similarity_mask
    ...
    if similarity_mask is not None:
        ...
        if torch.any(similarity_mask <= 0):
            raise ValueError("similarity_mask must contain only positive values before taking log")
