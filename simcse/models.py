...
def cl_forward(cls,
    ...
    if similarity_mask is not None:
        similarity_mask = similarity_mask.to(input_ids.device)
    ...
    if dist.is_initialized() and cls.training:
        ...
        if similarity_mask is not None:
            ...
            global_similarity_mask = similarity_mask.new_ones((global_batch_size, global_batch_size))
    ...
    if similarity_mask is not None:
        if similarity_mask.shape != cos_sim.shape:
            raise ValueError(
                f"similarity_mask shape {similarity_mask.shape} does not match logits shape {cos_sim.shape}"
            )
