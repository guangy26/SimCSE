...
def cl_forward(cls,
    ...
    if dist.is_initialized() and cls.training:
        if similarity_mask is not None:
            similarity_mask = similarity_mask.to(device=z1.device)
            ...
            local_mask_size = similarity_mask.size(0)
            global_similarity_mask = similarity_mask.new_ones(
                (local_mask_size * dist.get_world_size(), local_mask_size * dist.get_world_size())
            )
            for rank, rank_similarity_mask in enumerate(similarity_mask_list):
                start = rank * local_mask_size
                end = start + local_mask_size
