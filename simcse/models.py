...
def cl_forward(cls,
    ...
    if similarity_mask is not None:
        ...
        if (
            dist.is_initialized()
            and cls.training
            and similarity_mask.dim() == 2
            and similarity_mask.shape != cos_sim.shape
        ):
            local_rows, local_cols = similarity_mask.shape
            if local_rows * dist.get_world_size() == cos_sim.size(0) and local_cols * dist.get_world_size() == cos_sim.size(1):
