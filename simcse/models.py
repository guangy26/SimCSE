...
def cl_forward(cls,
    ...
    if dist.is_initialized() and cls.training:
        ...
        if similarity_mask is not None:
            ...
            # Each rank only computes helper-model scores for its local
            # examples. Keep cross-rank entries neutral.
            global_similarity_mask = similarity_mask.new_ones((z1.size(0), z2.size(0)))
            for mask_rank, rank_similarity_mask in enumerate(similarity_mask_list):
                row_start = mask_rank * batch_size
                row_end = row_start + batch_size
                global_similarity_mask[row_start:row_end, row_start:row_end] = rank_similarity_mask
    ...
    if similarity_mask is not None:
        cos_sim = cos_sim + torch.log(
            similarity_mask.to(device=cos_sim.device, dtype=cos_sim.dtype)
        ) # similarity_mask: (bs, bs)
