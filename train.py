    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                original_attention_mask = batch["attention_mask"][:, 0, :] if "attention_mask" in batch else None
                similar_attention_mask = batch["attention_mask"][:, 1, :] if "attention_mask" in batch else None
                original_token_type_ids = batch["token_type_ids"][:, 0, :] if "token_type_ids" in batch else None
                similar_token_type_ids = batch["token_type_ids"][:, 1, :] if "token_type_ids" in batch else None

                helper_device = next(self.help_model.parameters()).device
                original_sentences = original_sentences.to(helper_device)
                similar_sentences = similar_sentences.to(helper_device)
                if original_attention_mask is not None:
                    original_attention_mask = original_attention_mask.to(helper_device)
                    similar_attention_mask = similar_attention_mask.to(helper_device)
                if original_token_type_ids is not None:
                    original_token_type_ids = original_token_type_ids.to(helper_device)
                    similar_token_type_ids = similar_token_type_ids.to(helper_device)

                def pool_last_hidden(last_hidden_state, attention_mask):
                ...
                with torch.no_grad():
                    original_outputs = self.help_model(
                        ...
                        token_type_ids=original_token_type_ids,
                    )
                    similar_outputs = self.help_model(
                        similar_sentences,
                        attention_mask=similar_attention_mask,
                        token_type_ids=similar_token_type_ids,
                    )
                    original_embeddings = pool_last_hidden(original_outputs.last_hidden_state, original_attention_mask)
                    similar_embeddings = pool_last_hidden(similar_outputs.last_hidden_state, similar_attention_mask)

                # Thresholds are raw cosine values; do not divide by the contrastive temperature here.
                ...
                diagonal_mask = torch.eye(bs, dtype=torch.bool, device=similarity_scores.device)
                mask_greater = (similarity_scores > self.similarity_threshold_high) & ~diagonal_mask
                mask_lower = similarity_scores < self.similarity_threshold_low
                
                # TODO: 添加一个flag用于表示, sim_scores是否使用动态掩码
                similarity_scores = similarity_scores.clamp_min(torch.finfo(similarity_scores.dtype).tiny)
                similarity_scores[mask_greater] = math.exp(-10)
                similarity_scores[mask_lower] = 1.0
                similarity_scores[diagonal_mask] = 1.0
                batch["similarity_mask"] = similarity_scores.to(batch["input_ids"].device)
        ...
        def mask_tokens(
            ...
            if self.tokenizer.mask_token is None:
                raise ValueError(
                    "This tokenizer does not have a mask token, so it cannot be used for masked language modeling."
                )
            ...
            if special_tokens_mask is None:
                ...
                special_tokens_mask = special_tokens_mask.bool().to(labels.device)
