    ...
    class OurDataCollatorWithPadding:
        ...
        def __post_init__(self):
            if self.help_model is not None:
                ...
                for parameter in self.help_model.parameters():
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                original_sentences = batch["input_ids"][:, 0, :].to(helper_device)
                similar_sentences = batch["input_ids"][:, 1, :].to(helper_device)
                original_attention_mask = batch["attention_mask"][:, 0, :].to(helper_device)
                similar_attention_mask = batch["attention_mask"][:, 1, :].to(helper_device)

                with torch.no_grad():
                    original_outputs = self.help_model(original_sentences, attention_mask=original_attention_mask)
                    similar_outputs = self.help_model(similar_sentences, attention_mask=similar_attention_mask)

                    original_mask = original_attention_mask.unsqueeze(-1).to(original_outputs.last_hidden_state.dtype)
                    similar_mask = similar_attention_mask.unsqueeze(-1).to(similar_outputs.last_hidden_state.dtype)
                    original_embeddings = (
                        original_outputs.last_hidden_state * original_mask
                    ).sum(dim=1) / original_mask.sum(dim=1).clamp(min=1e-12)
                    similar_embeddings = (
                        similar_outputs.last_hidden_state * similar_mask
                    ).sum(dim=1) / similar_mask.sum(dim=1).clamp(min=1e-12)
