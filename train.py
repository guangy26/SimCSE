            ...
            if self.help_model is not None:
                ...
                with torch.no_grad():
                    original_outputs = self.help_model(
                        ...
                        token_type_ids=batch.get("token_type_ids", None)[:, 0, :] if "token_type_ids" in batch else None,
                    )
                    similar_outputs = self.help_model(
                        similar_sentences,
                        attention_mask=batch["attention_mask"][:, 1, :],
                        token_type_ids=batch.get("token_type_ids", None)[:, 1, :] if "token_type_ids" in batch else None,
                    )

                original_embeddings = (
                    original_outputs.last_hidden_state * batch["attention_mask"][:, 0, :].unsqueeze(-1)
                ).sum(dim=1) / batch["attention_mask"][:, 0, :].sum(dim=1, keepdim=True).clamp_min(1)
                similar_embeddings = (
                    similar_outputs.last_hidden_state * batch["attention_mask"][:, 1, :].unsqueeze(-1)
                ).sum(dim=1) / batch["attention_mask"][:, 1, :].sum(dim=1, keepdim=True).clamp_min(1)
