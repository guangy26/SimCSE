...
def main():
    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                original_lengths = original_attention_mask.sum(dim=1).clamp(min=1).unsqueeze(-1)
                similar_lengths = similar_attention_mask.sum(dim=1).clamp(min=1).unsqueeze(-1)
                original_embeddings = (
                    original_outputs.last_hidden_state * original_attention_mask.unsqueeze(-1)
                ).sum(dim=1) / original_lengths
                similar_embeddings = (
                    similar_outputs.last_hidden_state * similar_attention_mask.unsqueeze(-1)
                ).sum(dim=1) / similar_lengths
                ...
                similarity_mask = torch.ones_like(similarity_scores)
                mask_greater = similarity_scores > self.similarity_threshold_high
                mask_middle = (
                    (similarity_scores >= self.similarity_threshold_low)
                    & (similarity_scores <= self.similarity_threshold_high)
                )

                # TODO: 添加一个flag用于表示, sim_scores是否使用动态掩码
                similarity_mask[mask_greater] = math.exp(-10)
                similarity_mask[mask_middle] = similarity_scores[mask_middle].clamp_min(1e-12)
                # Positive pairs are the diagonal targets and must never be suppressed.
        ...
        def mask_tokens(
            ...
            if special_tokens_mask is None:
                ...
                special_tokens_mask = special_tokens_mask.to(
                    device=labels.device,
                    dtype=torch.bool,
                )
            ...
            if self.tokenizer.pad_token_id is not None:
                probability_matrix.masked_fill_(labels.eq(self.tokenizer.pad_token_id), value=0.0)
            ...
            # Half of the remaining 20% become random tokens, giving the standard 10%.
            indices_random = (
    ...
    if model_args.help_model_path is not None:
        help_model = AutoModel.from_pretrained(model_args.help_model_path)
        help_model.eval()
