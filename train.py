...
def main():
    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                with torch.no_grad():
                    ...
                    original_embeddings = self.mean_pooling(
                        original_outputs.last_hidden_state,
                        original_attention_mask,
                    )
                    similar_embeddings = self.mean_pooling(
                ...
                mask_greater = mask_greater & ~diagonal
                
                # TODO: 添加一个flag用于表示, sim_scores是否使用动态掩码
                similarity_scores[mask_greater] = torch.tensor(math.exp(-10), device=similarity_scores.device)
                similarity_scores[mask_lower | diagonal] = torch.tensor(1.0, device=similarity_scores.device)
        ...
        def mean_pooling(last_hidden_state: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
            ...
            return torch.sum(last_hidden_state * input_mask_expanded, dim=1) / torch.clamp(
