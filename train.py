...
def main():
    ...
    class OurDataCollatorWithPadding:
        ...
        def _masked_mean_pool(self, outputs, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                original_token_type_ids = batch.get("token_type_ids", None)
                similar_token_type_ids = None
                if original_token_type_ids is not None:
                    similar_token_type_ids = original_token_type_ids[:, 1, :]
                    original_token_type_ids = original_token_type_ids[:, 0, :]
                
                with torch.no_grad():
                    original_kwargs = {"attention_mask": original_attention_mask}
                    similar_kwargs = {"attention_mask": similar_attention_mask}
                    if original_token_type_ids is not None:
                        original_kwargs["token_type_ids"] = original_token_type_ids
                        similar_kwargs["token_type_ids"] = similar_token_type_ids

                    # Use the frozen helper model to compute raw cosine scores for mask thresholds.
                    original_outputs = self.help_model(original_sentences, **original_kwargs)
                    similar_outputs = self.help_model(similar_sentences, **similar_kwargs)
                    original_embeddings = self._masked_mean_pool(original_outputs, original_attention_mask) # (bs, dim)
                    similar_embeddings = self._masked_mean_pool(similar_outputs, similar_attention_mask) # (bs, dim)
                    ...
                    # If the raw cosine score is greater than threshold_high, set it to e^-10.
                    # If it is less than threshold_low, set it to 1. Otherwise, keep the score.
                    ...
                    diag_idx = torch.arange(bs, device=similarity_mask.device)
                    similarity_mask[diag_idx, diag_idx] = 1.0
