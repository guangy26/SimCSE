    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                similar_embeddings = F.normalize(similar_embeddings, p=2, dim=1)
                ...
                similarity_mask[mask_greater] = similarity_mask.new_full((), math.exp(-10))
                similarity_mask[mask_lower] = similarity_mask.new_full((), 1.0)
                diag_len = min(similarity_mask.size(0), similarity_mask.size(1))
                diag_idx = torch.arange(diag_len, device=similarity_mask.device)
