    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                original_token_mask = original_attention_mask.unsqueeze(-1)
                similar_token_mask = similar_attention_mask.unsqueeze(-1)
                original_embeddings = (
                    original_outputs.last_hidden_state * original_token_mask
                ).sum(dim=1) / original_token_mask.sum(dim=1).clamp(min=1)
                similar_embeddings = (
                    similar_outputs.last_hidden_state * similar_token_mask
                ).sum(dim=1) / similar_token_mask.sum(dim=1).clamp(min=1)

                # Build a multiplicative logit mask from raw cosine scores.
                ...
                similarity_mask = similarity_scores.clamp(min=1e-12)
                ...
                diagonal = torch.arange(bs, device=similarity_mask.device)
        ...
        def mask_tokens(
            ...
            # The remaining 10% of masked tokens stay unchanged.
