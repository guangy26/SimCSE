    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                positive_pairs = torch.eye(bs, dtype=torch.bool, device=similarity_mask.device)
                similarity_mask = similarity_mask.masked_fill(positive_pairs, 1.0)
        ...
        def mask_tokens(
            ...
            if self.tokenizer.mask_token is None:
                raise ValueError(
                    "This tokenizer does not have a mask token, so masked language modeling cannot be used."
                )
