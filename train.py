    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                original_mask = original_attention_mask.unsqueeze(-1)
                similar_mask = similar_attention_mask.unsqueeze(-1)
                original_embeddings = (
                    original_outputs.last_hidden_state * original_mask
                ).sum(dim=1) / original_mask.sum(dim=1).clamp(min=1)
                similar_embeddings = (
                    similar_outputs.last_hidden_state * similar_mask
                ).sum(dim=1) / similar_mask.sum(dim=1).clamp(min=1)
