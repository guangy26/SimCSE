        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                if token_type_ids is not None:
                    original_inputs["token_type_ids"] = token_type_ids[:, 0, :]
                    similar_inputs["token_type_ids"] = token_type_ids[:, 1, :]

                with torch.no_grad():
                    original_outputs = self.help_model(**original_inputs)
                    similar_outputs = self.help_model(**similar_inputs)
                ...
                # Threshold raw cosine similarity; logits are temperature-scaled later in the loss.
