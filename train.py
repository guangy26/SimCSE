        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                def mean_pool(last_hidden_state: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
                    token_mask = attention_mask.unsqueeze(-1).to(last_hidden_state.dtype)
                    return (last_hidden_state * token_mask).sum(dim=1) / token_mask.sum(dim=1).clamp_min(1e-12)
                ...
                with torch.no_grad():
                    ...
                    min_mask_value = torch.finfo(raw_similarity.dtype).tiny
                    similarity_mask = torch.where(
                        raw_similarity > self.similarity_threshold_high,
                        raw_similarity.new_full(raw_similarity.shape, math.exp(-10)),
                        torch.where(
                            raw_similarity < self.similarity_threshold_low,
                            torch.ones_like(raw_similarity),
                            raw_similarity.clamp_min(min_mask_value),
                        ),
                    )
