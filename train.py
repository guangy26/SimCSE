    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                with torch.no_grad():
                    original_hidden = self.help_model(
                        original_sentences,
                        attention_mask=original_attention_mask,
                    ).last_hidden_state
                    similar_hidden = self.help_model(
                        similar_sentences,
                ...
                original_embeddings = (original_hidden * original_attention_mask.unsqueeze(-1)).sum(dim=1) / original_lengths
                similar_embeddings = (similar_hidden * similar_attention_mask.unsqueeze(-1)).sum(dim=1) / similar_lengths
                ...
                similarity_scores = torch.where(
                    mask_greater,
                    torch.full_like(similarity_scores, math.exp(-10)),
                    similarity_scores,
                )
                similarity_scores = torch.where(
                    mask_lower,
                    torch.ones_like(similarity_scores),
                    similarity_scores,
                )
                diagonal_indices = torch.arange(similarity_scores.size(0), device=similarity_scores.device)
                similarity_scores[diagonal_indices, diagonal_indices] = 1.0
