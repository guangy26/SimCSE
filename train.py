        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                similarity_mask = torch.where(
                    ...
                    torch.ones_like(similarity_mask),
                    similarity_mask,
                )
                diag = torch.arange(similarity_mask.size(0), device=similarity_mask.device)
                similarity_mask[diag, diag] = 1.0
                batch["similarity_mask"] = similarity_mask.detach()
