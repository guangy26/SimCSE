        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                def mean_pool(outputs, attention_mask):
                    ...
                    input_mask_expanded = attention_mask.unsqueeze(-1).to(last_hidden.dtype)
                    return (
                        (last_hidden * input_mask_expanded).sum(dim=1)
                        / input_mask_expanded.sum(dim=1).clamp(min=1e-12)
                    )
                ...
                # Threshold on raw cosine scores; the training loss applies temperature separately.
                ...
                similarity_scores = similarity_scores.masked_fill(mask_greater, math.exp(-10))
                similarity_scores = similarity_scores.masked_fill(mask_lower, 1.0)
                similarity_scores.fill_diagonal_(1.0)
