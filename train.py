    ...
    class OurDataCollatorWithPadding:
        ...
        def mask_tokens(
            ...
            if self.tokenizer.mask_token is None:
                raise ValueError(
                    "This tokenizer does not have a mask token, so MLM training cannot be used."
                )

            labels = inputs.clone()
            probability_matrix = torch.full(
                labels.shape, self.mlm_probability, device=labels.device
            )
        ...
        def _mean_pool(self, last_hidden_state: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
            ...
            return (
                (last_hidden_state * input_mask_expanded).sum(1)
                / input_mask_expanded.sum(1).clamp_min(torch.finfo(last_hidden_state.dtype).eps)
            )
