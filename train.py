    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                # Use the frozen helper model only to build a static loss mask.
                with torch.no_grad():
                    ...
                    original_embeddings = (
                        original_outputs * original_attention_mask.unsqueeze(-1)
                    ).sum(dim=1) / original_attention_mask.sum(dim=1, keepdim=True).clamp_min(1)
                    similar_embeddings = (
                        similar_outputs * similar_attention_mask.unsqueeze(-1)
                    ).sum(dim=1) / similar_attention_mask.sum(dim=1, keepdim=True).clamp_min(1)
                ...
                similarity_scores[mask_greater] = similarity_scores.new_tensor(math.exp(-10))
                similarity_scores[mask_lower] = similarity_scores.new_tensor(1.0)
