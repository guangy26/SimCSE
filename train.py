    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                if mask_greater.size(0) == mask_greater.size(1):
                    diagonal = torch.eye(mask_greater.size(0), dtype=torch.bool, device=mask_greater.device)
                    mask_greater = mask_greater.masked_fill(diagonal, False)
        ...
        def _mean_pool(self, outputs, attention_mask):
            ...
            return (outputs.last_hidden_state * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1e-9)
        ...
        def mask_tokens(
            ...
            if self.tokenizer.mask_token is None:
                raise ValueError(
                    "This tokenizer does not have a mask token, which is required when --do_mlm is enabled."
                )
