    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                non_diagonal = ~torch.eye(bs, dtype=torch.bool, device=similarity_scores.device)
                mask_greater = (similarity_scores > self.similarity_threshold_high) & non_diagonal

                # Penalize only high-similarity negatives; positives and neutral pairs leave logits unchanged.
