    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                def helper_sentence_embeddings(sentence_ids, sentence_attention_mask, sentence_token_type_ids=None):
                    helper_inputs = {
                        "input_ids": sentence_ids.to(helper_device),
                        "attention_mask": sentence_attention_mask.to(helper_device),
                    }
                    if sentence_token_type_ids is not None:
                        helper_inputs["token_type_ids"] = sentence_token_type_ids.to(helper_device)

                    helper_outputs = self.help_model(**helper_inputs)
                    hidden_states = helper_outputs.last_hidden_state
                    mask = helper_inputs["attention_mask"].unsqueeze(-1).to(hidden_states.dtype)
                    return (hidden_states * mask).sum(dim=1) / mask.sum(dim=1).clamp_min(1.0)

                token_type_ids = batch.get("token_type_ids")
                original_token_type_ids = token_type_ids[:, 0, :] if token_type_ids is not None else None
                similar_token_type_ids = token_type_ids[:, 1, :] if token_type_ids is not None else None

                with torch.no_grad():
                    original_embeddings = helper_sentence_embeddings(
                        batch["input_ids"][:, 0, :],
                        batch["attention_mask"][:, 0, :],
                        original_token_type_ids,
                    )
                    similar_embeddings = helper_sentence_embeddings(
                        batch["input_ids"][:, 1, :],
                        batch["attention_mask"][:, 1, :],
                        similar_token_type_ids,
                    )

                original_embeddings = torch.nn.functional.normalize(original_embeddings, p=2, dim=-1)
                similar_embeddings = torch.nn.functional.normalize(similar_embeddings, p=2, dim=-1)
                similarity_scores = torch.matmul(original_embeddings, similar_embeddings.transpose(0, 1))

                similarity_mask = similarity_scores.clone()
                similarity_mask[similarity_scores > self.similarity_threshold_high] = math.exp(-10)
                similarity_mask[similarity_scores < self.similarity_threshold_low] = 1.0
                similarity_mask.fill_diagonal_(1.0)
                similarity_mask.clamp_(min=torch.finfo(similarity_mask.dtype).tiny)
                batch["similarity_mask"] = similarity_mask.to(batch["input_ids"].device)
