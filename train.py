    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if self.help_model is not None:
                ...
                with torch.no_grad():
                    original_embeddings = mean_pool(
                        self.help_model(original_sentences, attention_mask=original_attention_mask),
                        original_attention_mask,
                    )
                    similar_embeddings = mean_pool(
                        self.help_model(similar_sentences, attention_mask=similar_attention_mask),
                ...
                false_positive_mask = similarity_scores > self.similarity_threshold_high
                false_positive_mask.fill_diagonal_(False)
                similarity_mask[false_positive_mask] = math.exp(-10)
